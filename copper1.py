...
if selected  == "Menu":
        ...
        with tab1:
            ...
            if submit_button and flag == 0:
                ...
                # Ensure you have the same categories used during training
                item_type_categories = ['W', 'WI', 'S', 'Others', 'PL', 'IPL', 'SLAWR']

                # Update the OneHotEncoder initialization with the categories parameter
                encoder = OneHotEncoder(sparse=False, categories=[item_type_categories], drop='first', handle_unknown='error', 
                                        dtype='float64', feature_names_out=feature_names)
                
                encoder.fit(new_sample[:, [7, 8, 9]])
                new_sample_ohe = encoder.transform(new_sample[:, [7, 8, 9]])
